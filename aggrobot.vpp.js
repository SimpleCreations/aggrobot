        ...
        if (request != null) switch (request.type) {
            ...
            case AggroBot.Request.Type.TEXT:
                ...
                if (AggroBot.vkEnabled) {
                    let matches;
                    if (/(кинь|скажи|напиши|пришли|дай|давай|([^а-яё]|^)го|отправь|черкани|сыл(ку|ь)( на)?|линк(ани)?|записан|может|страницу)([ \-]?ка)?( тогда)?( сво[йю])?( ты| в| мне)?( тогда)?( сво[йю])? (вк|vk|id|ай ?[дп]и|стр(?=[^а-я]|$|ани)|одноклас+ники|фб|fb|фейсбук|facebook|телег|в(ай|и)бер|в[оа](тс|ц)ап)|(вк|vk|id|ай ?[дп]и|одноклас+ники|фб|fb|фейсбук|facebook|телег(у|рам+)|в(ай|и)бер|в[оа](тс|ц)ап+)( ты| мне)?( свой)? (с?кинь|скажи|напиши|пришли|дай|давай|го|отправь|черкани|с+ыл(ку|ь)|линк)/i.test(request.text) ||
...
Object.assign(AggroBot, {
    ...
    getVKRequestProbability(amountOfRequests) {
        return 1 / (12 * (amountOfRequests / 2 + 1));
    }
